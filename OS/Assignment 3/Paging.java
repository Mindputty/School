...
public class Paging {
	...
	private int clockPointer; // the pointer for use in the clock page selection algorithm
	...
	public void init(int numFrames, int numPages, byte offbts, long block) {
		...
		clockPointer = 0; //clock pointer starts at pageBuffer index 0
	...
	// mem
	{
			...
			// in memory P bit
		{
			...
			//tell the used bit that this page is about to be used in the Kernel READ or WRITE function
	...
	public int replacePageLRU(int replacePageNum) {
		int oldPageNum = pageTable[pageBuffer[0]].vPage;
		int freedFrame;
		int leastRecentTime = 999999;
		int lruBufferIndex = 0;
		
		// an algorithm for finding the LRU page
		// search the pageTable for the entry in pageBuffer that has the largest lastTouchTime and save that page for replacement
		for(int i=0; i<pageBuffer.length; i++){
			if(leastRecentTime > pageTable[pageBuffer[i]].lastTouchTime){
				leastRecentTime = pageTable[pageBuffer[i]].lastTouchTime;
				oldPageNum = pageBuffer[i];
				lruBufferIndex = i;
			}
		}
		
		pageBuffer[lruBufferIndex] = replacePageNum; // replace it
	...
	// the Clock page selection and replacement algorithm
	public int addPageClock(int virtualPageNum) {
		
		int removePageNum = 0;
		int i = 0;

		while(i <= pageBuffer.length){
			if(pageTable[pageBuffer[clockPointer]].u == 1){
				pageTable[pageBuffer[clockPointer]].u = 0;
				clockPointer++;
				i++;
				
				if (clockPointer == numPhysicalPages){
					clockPointer = 0; // cycle to start if at end
				}
			} else {
				removePageNum = pageBuffer[clockPointer]; // get value at pointer
				pageBuffer[clockPointer] = virtualPageNum; // replace it
				break;
			}
		}
	...
	public int replacePageCLOCK(int replacePageNum) {
		...
		oldPageNum = addPageClock(replacePageNum); // use the clock selection algorithm to replace the page
		freedFrame = pageTable[oldPageNum].frame; // get frame number
